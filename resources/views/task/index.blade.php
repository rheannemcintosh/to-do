<x-layout>
    ...
    @endif
    <div class=" min-w-screen min-h-screen bg-gray-100 flex justify-center bg-gray-100 font-sans">
        <div class="w-full">
            <div class="bg-white overflow-x-scroll shadow-md rounded my-6">
                <table class="font-poppins min-w-max w-full table-auto text-sm">
                    <thead>
                        <tr class="bg-tdTeal text-white uppercase text-sm leading-normal">
                    ...
                    @foreach($tasks as $task)
                        <tr class="border-b border-gray-200 hover:bg-gray-100">
                            ...
                            <td class="py-3 px-6 text-left text-xs whitespace-nowrap">
                                <div class="font-bold">@if (strlen($task->task) >= 50) {{ substr($task->task, 0, 50 ).'. . .'}} @else {{ $task->task }}@endif</div>
                                <div class="text-2xs">@if (strlen($task->description) >= 50) {{ substr($task->description, 0, 50 ).'. . .'}} @else {{ $task->description }}@endif</div>
                            </td>
